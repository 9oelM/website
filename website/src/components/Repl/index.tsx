...
export const Repl = () => {
  ...
  const [loadingImportsStatus, setLoadingImportsStatus] = useState<'loading' | 'success' | 'fail'>('loading');
  ...
  const swcWasm: MutableRefObject<typeof import('@swc/wasm-web') | null> = useRef(null);
  const CodeMirrorLazyImported: MutableRefObject<(typeof import('react-codemirror2')) | null> = useRef(null);
  const RawCodeEditor: MutableRefObject<CodeMirror.Editor | null> = useRef(null);
  const CompiledCodeEditor: MutableRefObject<CodeMirror.Editor | null> = useRef(null);
  ...
  const onRawCodeChange: ComponentPropsWithRef<typeof CodeMirror>['onBeforeChange'] = useCallback((_editor, _data, value) => {
    setRawCode(value);
  }, []);

  const onRawCodeEditorDidMount: ComponentPropsWithRef<typeof CodeMirror>['editorDidMount'] = useCallback((editor) => {
    CompiledCodeEditor.current = editor;
    setEditorsLoadedCount((prev) => prev + 1);
  }, []);

  const onCompiledCodeEditorDidMount: ComponentPropsWithRef<typeof CodeMirror>['editorDidMount'] = useCallback((editor) => {
